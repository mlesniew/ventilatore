...
void setup() {
  ...
  {
    const auto config =
        PicoUtils::JsonConfigFile<JsonDocument>(LittleFS, FPSTR(CONFIG_FILE));
...

  {
    ...
    struct XORInput : PicoUtils::BinaryInput {
      XORInput(PicoUtils::BinaryInput& a, PicoUtils::BinaryInput& b)
          : a(a), b(b) {}
    ...
    wifi_control.get_connectivity_level = [] {
      return 1 + (fan_control.healthcheck() ? 1 : 0) +
             (HomeAssistant::connected() ? 1 : 0);
    };
  }
  ...
  server.on("/status", [] { server.sendJson(fan_control.get_json()); });
...
PicoUtils::PeriodicRun switch_proc(0.25, [] {
  ...
  if (switch_position != last_switch_position) {
    fan_control.mode =
        fan_control.is_fan_running() ? FanControl::OFF : FanControl::ON;
  }
