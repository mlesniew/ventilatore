...
FanControl::FanControl(PicoUtils::BinaryOutput& relay, const Settings& settings,
                       PicoMQ& picomq)
    : mode(OFF),
      fan_running(false),
      relay(relay),
...
void FanControl::init() {
  if (settings.sensor.length()) {
    picomq.subscribe("celsius/+/" + settings.sensor + "/humidity",
                     [this](String payload) {
...
void FanControl::process(bool respect_min_times) {
  if (mode != AUTO) {
    ...
    if (mode.elapsed_millis() >=
        settings.fan.force_timeout_minutes * 60 * 1000) {
      logger.println(F("Switching back to automatic mode..."));
      mode = AUTO;
    }
  }

  const auto auto_off_humidity =
      settings.fan.humidity - settings.fan.hysteresis / 2;
  const auto auto_on_humidity =
      settings.fan.humidity + settings.fan.hysteresis / 2;
  ...
  if (mode == AUTO) {
    ...
    if (fan_running) {
      ...
      const bool max_time_elapsed =
          (elapsed >= settings.fan.max_auto_on_time) &&
          (settings.fan.max_auto_on_time > 0);
      ...
      const bool max_time_elapsed =
          (elapsed >= settings.fan.max_auto_off_time) &&
          (settings.fan.max_auto_off_time > 0);
